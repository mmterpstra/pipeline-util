...
perl $0 -d <human.dict> <varscan.called> -v <vcf.table> #also plot f/z vals
#note: the 
my actual use:
...
#plot
plot(VarscanDat\$offset, VarscanDat\$adjusted_log_ratio, col="grey", pch='.', lty=3, xlab="", ylab="log2(tumor/normal)", xaxt="n", main="$shortname" , ylim = c(-3,2))
...
END
	...
	while ( $i < $ChrLenData{'REFcount'} ) {
		...
		while(defined($ChrLenData{'order'}{($i + 1)}) && $minorTickPos < $ChrLenData{'ChrLen'}{ $ChrLenData{'order'}{($i )} }){
...
pdf(file="$dataTable.pdf", width=20, height=6, useDingbats=F)
#plot
plot(VarscanIntervalDat\$offset, VarscanIntervalDat\$adjusted_log_ratio, col="grey", pch='.', lty=3, xlab="", ylab="log2(tumor/normal)", xaxt="n", main="$shortname")
abline(a=log(x=2,base=2),b=0, col="red")
abline(a=log(x=1.5,base=2),b=0, col="orange")
abline(a=log(x=1,base=2),b=0, col="green")
abline(a=log(x=0.5,base=2),b=0, col="blue")
...
sub MakeVCFDataTable {
	warn "MakeVCFDataTable input:".$_[0];
...
sub MakePDFrscriptWithVCF2 {
	my $dataTable = shift @_;
	my $shortname = basename($dataTable,'.cn.called','.Varscan2copyCaller.called','.40.called','.called','.Varscan2copyCaller','.table');
	my $dirname = dirname($dataTable);
	my $variantTable = shift @_;
	my $segTable = shift @_;
	my %ChrLenData = @_;
...
plot(VarscanDat\$offset, VarscanDat\$adjusted_log_ratio, col="grey", pch='.', lty=3, xlab="", ylab="log2(tumor/normal)", xaxt="n", main="$shortname" , ylim = c(-3,2), xlim = c(1,$ChrLenData{'len'}))
abline(a=log(x=2,base=2),b=0, col="red")
abline(a=log(x=1.5,base=2),b=0, col="orange")
abline(a=log(x=1,base=2),b=0, col="green")
abline(a=log(x=0.5,base=2),b=0, col="blue")
...
segments(DNAcopySegDat\$loc.start,DNAcopySegDat\$seg.mean,DNAcopySegDat\$loc.end,DNAcopySegDat\$seg.mean, col="black")
dev.off()
END
...
layout(matrix(c(1,2), 2, 1, byrow = TRUE),
    heights=c(1,1))
#plot
plot(VarscanIntervalDat\$offset, VarscanIntervalDat\$adjusted_log_ratio, col="grey", pch='.', lty=3, xlab="", ylab="log2(tumor/normal)", xaxt="n", main="$shortname" , xlim=c(1,($end-$start)))
segments(DNAcopySegDat\$offsetStart,DNAcopySegDat\$seg.mean,DNAcopySegDat\$offsetEnd,DNAcopySegDat\$seg.mean, col="black")
abline(a=log(x=2,base=2),b=0, col="red")
abline(a=log(x=1.5,base=2),b=0, col="orange")
abline(a=log(x=1,base=2),b=0, col="green")
abline(a=log(x=0.5,base=2),b=0, col="blue")
