...
sub main {
	...
	getopts('r:b:i:o:', \%{$opts});
	#warn Dumper($opts);
	if(not(defined($opts -> {'r'}) && defined($opts -> {'b'}) && -e $opts -> {'b'} && defined($opts -> {'i'}) && -e $opts -> {'i'} && defined($opts -> {'o'}))){
...
sub wrapper {
	...
	#good luck at debug
	my $cmd="set -x;set -e && bwa mem ".$opts -> {'r'}." ".$opts -> {'i'}." > " . $opts -> {'t'} . ".tmp.sam" . "&&  mkfifo ".$opts -> {'t'}.".samfifo;".
...
sub usemsg {
	my $use =<<"END";
use: $0 -r BWAINDEXBASE -b TRIMBED -i INFASTQGZ -o OUTFASTQGZ
