...
####################################################################################################################
	...
	for my $sample (@samples){
		my @sampleAD =  split(',',$formatInfo{$sample}{'AD'}) if $formatInfo{$sample}{'GT'} ne './.' && defined($formatInfo{$sample}{'AD'});
		
		my @Gt = split('/',$formatInfo{$sample}{'GT'});
		if(($formatInfo{$sample}{'GT'} ne './.') && defined($formatInfo{$sample}{'AD'}) && (Sum(@sampleAD) > 0 ) ){
		...
		}elsif($formatInfo{$sample}{'GT'} eq './.'){
			$formatInfo{$sample}{'F'} = '.';
		}elsif(not(defined($formatInfo{$sample}{'AD'}))){
			$formatInfo{$sample}{'F'} = '.';#could be just 0 ?
		}
	}
