...
use  Getopt::Long;
my $opts ={};
my $result = GetOptions ("caller=s"	=> \$opts -> {'c'},
                        "input=s"	=> \$opts -> {'i'},
			"output=s"	=> \$opts -> {'o'},
			"prefixedonly"	=> \$opts -> {'p'},
);
...
sub main{
	...
	CalleriseInfoHeader('vcf' => $vcf, 'caller' => $caller, 'opts'=> $opts);
	CalleriseFormatHeader('vcf' => $vcf, 'caller' => $caller, 'opts'=> $opts);
...
#		}
		CalleriseRecordInfoData('record' => $x,'caller' => $caller, 'opts'=> $opts);
		CalleriseRecordFormatData('record' => $x,'caller' => $caller, 'opts'=> $opts);
		#die Dumper $x;
...
sub CalleriseInfoHeader {
	...
	for my $field (keys(%{$vcf -> {header} -> {INFO}})){
		...
		delete($vcf -> {header} -> {'INFO'} -> {$field}) if($opts -> {'p'})
	}
}
...
sub CalleriseRecordInfoData {
	...
	for my $field (keys(%{$record -> {'INFO'}})){
		...
		delete($record -> {'INFO'} -> {$field})if($opts -> {'p'});
	}
}
sub CalleriseRecordFormatData {
	...
	for my $sample (keys(%{$record -> {'gtypes'}})){
		for my $field (keys(%{$record -> {'gtypes'} -> {$sample}})){
			...
			delete($record -> {'gtypes'} -> {$sample} -> {$field})if($opts -> {'p'});
		}
