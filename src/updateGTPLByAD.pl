...
sub HasFunkyGTPL {
	...
	if(GetPL('record' => $x, 'sample' => $sample) ){
		...
		if(GetSet('record' => $x) eq 'freebayes' && (GetGT('record' => $x, 'sample' => $sample) ) && GetGT('record' => $x, 'sample' => $sample) ne './.'){
			if( (min(@PL)==0) && ($PL[GetIndexGT(GetGT('record' => $x, 'sample' => $sample))] == 0)){
				return 0;
			}else{
				warn Dumper($x);
				return 1;
...
sub GetIndexGT {
	my $gt = shift @_;
	die "Clean Gt input" if($gt eq './.' || $gt eq '.');
	my $idx=0;
	my $a1 = 0;
	my $a2 = 0;
	while($idx < 50){
		...
		return $idx if($gt eq $a1.'/'.$a2);
		$a1++;
		$idx++;
	}
	die "ran out if options"
}
